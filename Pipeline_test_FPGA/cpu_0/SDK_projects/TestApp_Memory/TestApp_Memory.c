...
int main (void) {
 ...
 // Object 1 
   ...
   Xuint32 read_statue; // 1: fifo is valid, 0: fifo is unvalid
   Xuint32 read_eof;  //
  ...
  matrix[3][0] = 0.0; matrix[3][1] = 0.0; matrix[3][2] = 0.0; matrix[3][3] = 1.0; 
    ...
    */
    ...
    XIo_Out32(vertex_texcoord, 1); // the posedge 
  ...
  while (1)
	...
	{	
		...
		while(read_eof==0){read_eof = XIo_In32(GPU_STATUE);}
	   		XIo_Out32(clc_scr, 0); 
      		XIo_Out32(clc_scr, 1); // the positive clear screen
	        count=count+1 ;
  ...
  for (i=0; i<=3; i++) { 
    for (j=0; j<=3; j++) { 
      ...
      if (j==0) { 
		...
		XIo_Out32(vertex_texcoord, 0);//0 choose for texture color; 1 choose for vertex color;
		XIo_Out32(clc_scr, 0);  //the negtive stop clear screen;
